/**
 * WGS84 geodesy utilities: LLH ↔ ECEF ↔ ENU ↔ NED.
 *
 * The internal world frame is NED (North, East, Down). GPS positions are
 * converted from geodetic (lat/lon/alt MSL) to NED metres via ECEF+ENU,
 * then ENU is converted to NED in a single step.
 *
 * Formulas are WGS84 standard; the rotation matrix rows are verified against
 * Betaflight firmware conventions (see plan.md frames section).
 */

const WGS84_A = 6378137.0;
const WGS84_F = 1.0 / 298.257223563;
const WGS84_E2 = WGS84_F * (2 - WGS84_F);

/** Degrees to radians */
function deg2rad(d) {
    return (d * Math.PI) / 180;
}

/** Radians to degrees */
function rad2deg(r) {
    return (r * 180) / Math.PI;
}

/**
 * Geodetic (WGS84) to Earth-Centered-Earth-Fixed.
 *
 * @param {number} lat - degrees
 * @param {number} lon - degrees
 * @param {number} alt - metres above WGS84 ellipsoid (MSL)
 * @returns {{ x: number, y: number, z: number }} ECEF position in metres
 */
export function llhToEcef(lat, lon, alt) {
    const rlat = deg2rad(lat);
    const rlon = deg2rad(lon);
    const sinLat = Math.sin(rlat);
    const cosLat = Math.cos(rlat);
    const sinLon = Math.sin(rlon);
    const cosLon = Math.cos(rlon);
    const N = WGS84_A / Math.sqrt(1 - WGS84_E2 * sinLat * sinLat);
    return {
        x: (N + alt) * cosLat * cosLon,
        y: (N + alt) * cosLat * sinLon,
        z: (N * (1 - WGS84_E2) + alt) * sinLat,
    };
}

/**
 * ECEF to ENU (East, North, Up) local tangent plane about an origin.
 *
 * @param {{ x: number, y: number, z: number }} point - target ECEF position
 * @param {number} lat0 - origin latitude, degrees
 * @param {number} lon0 - origin longitude, degrees
 * @param {number} alt0 - origin altitude, metres
 * @returns {{ e: number, n: number, u: number }} ENU position in metres
 */
export function ecefToEnu(point, lat0, lon0, alt0) {
    const origin = llhToEcef(lat0, lon0, alt0);
    const dx = point.x - origin.x;
    const dy = point.y - origin.y;
    const dz = point.z - origin.z;

    const rlat0 = deg2rad(lat0);
    const rlon0 = deg2rad(lon0);
    const sinLat = Math.sin(rlat0);
    const cosLat = Math.cos(rlat0);
    const sinLon = Math.sin(rlon0);
    const cosLon = Math.cos(rlon0);

    return {
        e: -sinLon * dx + cosLon * dy,
        n: -sinLat * cosLon * dx - sinLat * sinLon * dy + cosLat * dz,
        u: cosLat * cosLon * dx + cosLat * sinLon * dy + sinLat * dz,
    };
}

/**
 * ENU to NED.
 *
 * NED = (ENU.north, ENU.east, −ENU.up)
 *
 * @param {{ e: number, n: number, u: number }} enu
 * @returns {{ n: number, e: number, d: number }} NED position in metres
 */
export function enuToNed(enu) {
    return { n: enu.n, e: enu.e, d: -enu.u };
}

/**
 * Geodetic to NED (convenience, wrapping the above).
 *
 * @param {number} lat - degrees
 * @param {number} lon - degrees
 * @param {number} alt - metres MSL
 * @param {number} lat0 - origin latitude, degrees
 * @param {number} lon0 - origin longitude, degrees
 * @param {number} alt0 - origin altitude, metres
 * @returns {{ n: number, e: number, d: number }} NED position in metres
 */
export function llhToNed(lat, lon, alt, lat0, lon0, alt0) {
    const ecef = llhToEcef(lat, lon, alt);
    const enu = ecefToEnu(ecef, lat0, lon0, alt0);
    return enuToNed(enu);
}

/**
 * NED to geodetic using Bowring's iterative inverse.
 *
 * @param {{ n: number, e: number, d: number }} ned - NED position in metres
 * @param {number} lat0 - origin latitude, degrees
 * @param {number} lon0 - origin longitude, degrees
 * @param {number} alt0 - origin altitude, metres
 * @param {number} [iterations=5]
 * @returns {{ lat: number, lon: number, alt: number }}
 */
export function nedToLlh(ned, lat0, lon0, alt0, iterations = 5) {
    const enu = { e: ned.e, n: ned.n, u: -ned.d };

    const originEcef = llhToEcef(lat0, lon0, alt0);
    const rlat0 = deg2rad(lat0);
    const rlon0 = deg2rad(lon0);
    const sinLat = Math.sin(rlat0);
    const cosLat = Math.cos(rlat0);
    const sinLon = Math.sin(rlon0);
    const cosLon = Math.cos(rlon0);

    const enuVec = [enu.e, enu.n, enu.u];
    const RT = [
        [-sinLon, -sinLat * cosLon, cosLat * cosLon],
        [cosLon, -sinLat * sinLon, cosLat * sinLon],
        [0, cosLat, sinLat],
    ];

    const x = originEcef.x + RT[0][0] * enuVec[0] + RT[0][1] * enuVec[1] + RT[0][2] * enuVec[2];
    const y = originEcef.y + RT[1][0] * enuVec[0] + RT[1][1] * enuVec[1] + RT[1][2] * enuVec[2];
    const z = originEcef.z + RT[2][0] * enuVec[0] + RT[2][1] * enuVec[1] + RT[2][2] * enuVec[2];

    const p = Math.sqrt(x * x + y * y);
    let lat = Math.atan2(z, p * (1 - WGS84_E2));
    let N, alt;

    for (let i = 0; i < iterations; i++) {
        const sinLatI = Math.sin(lat);
        N = WGS84_A / Math.sqrt(1 - WGS84_E2 * sinLatI * sinLatI);
        alt = p / Math.cos(lat) - N;
        lat = Math.atan2(z, p * (1 - WGS84_E2 * N / (N + alt)));
    }

    const lon = Math.atan2(y, x);

    return {
        lat: rad2deg(lat),
        lon: rad2deg(lon),
        alt,
    };
}

/**
 * Haversine great-circle distance (metres).
 *
 * @param {number} lat1 - degrees
 * @param {number} lon1 - degrees
 * @param {number} lat2 - degrees
 * @param {number} lon2 - degrees
 * @returns {number} distance in metres
 */
export function haversine(lat1, lon1, lat2, lon2) {
    const R = 6371000;
    const dLat = deg2rad(lat2 - lat1);
    const dLon = deg2rad(lon2 - lon1);
    const a =
        Math.sin(dLat / 2) * Math.sin(dLat / 2) +
        Math.cos(deg2rad(lat1)) *
            Math.cos(deg2rad(lat2)) *
            Math.sin(dLon / 2) *
            Math.sin(dLon / 2);
    const c = 2 * Math.atan2(Math.sqrt(a), Math.sqrt(1 - a));
    return R * c;
}

/**
 * Geographic bearing from point A to point B.
 *
 * @param {number} lat1 - degrees
 * @param {number} lon1 - degrees
 * @param {number} lat2 - degrees
 * @param {number} lon2 - degrees
 * @returns {number} bearing in degrees (0 = North, clockwise)
 */
export function bearing(lat1, lon1, lat2, lon2) {
    const rlat1 = deg2rad(lat1);
    const rlat2 = deg2rad(lat2);
    const dLon = deg2rad(lon2 - lon1);
    const y = Math.sin(dLon) * Math.cos(rlat2);
    const x =
        Math.cos(rlat1) * Math.sin(rlat2) -
        Math.sin(rlat1) * Math.cos(rlat2) * Math.cos(dLon);
    return ((rad2deg(Math.atan2(y, x)) % 360) + 360) % 360;
}

/**
 * Local tangent offset for small displacements (triad endpoints).
 *
 * Converts a NED offset vector to lat/lon/alt deltas using the flat-earth
 * approximation, which is exact at the few-metre scale of triad lines.
 *
 * @param {{ n: number, e: number, d: number }} offset - NED offset in metres
 * @param {number} lat - reference latitude, degrees
 * @returns {{ dLat: number, dLon: number, dAlt: number }}
 */
export function localTangentOffset(offset, lat) {
    const M_PER_DEG_LAT = 111320;
    const cosLat = Math.cos(deg2rad(lat));
    const dLat = offset.n / M_PER_DEG_LAT;
    const dLon = offset.e / (M_PER_DEG_LAT * cosLat);
    const dAlt = -offset.d;
    return { dLat, dLon, dAlt };
}

export { WGS84_A, WGS84_F, WGS84_E2 };
