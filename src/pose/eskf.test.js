...
import { createGpsPositionFactor, createGpsVelocityFactor, createQuaternionPrior } from "./measurements.js";
...
describe("eskf — forward pass on synthetic data", () => {
    it("recovers a straight trajectory from noisy GPS + perfect IMU", () => {
        ...
        // Ground truth at t=0
        const p0 = [traj[0].pNed.n, traj[0].pNed.e, traj[0].pNed.d];
        const v0 = [traj[0].vNed.n, traj[0].vNed.e, traj[0].vNed.d];
        const q0 = traj[0].q;

        // Initialize ESKF
        const eskf = createEskf({
            p0,
            v0: [v0[0] + randn(rng, 0, 1), v0[1] + randn(rng, 0, 1), v0[2] + randn(rng, 0, 0.5)],
            q0,
            sigmaPos: 5,
            sigmaVel: 2,
            sigmaAtt: 0.1,
        ...
        for (let i = 0; i < traj.length; i++) {
            ...
            // IMU simulation: for level flight, accel reads [0, 0, +g] (MEMS convention: reaction force)
            // Gyro is zero (straight line, no rotation)
            const accelBody = [0, 0, 9.80665];
            const gyro = [0, 0, 0];

            // Predict
            eskfPredict(eskf, gyro, accelBody, dt);

            // GPS update every 10th step (~10 Hz)
            if (i % 10 === 0) {
                const gpsSigma = 2.0;
                const gpsMeas = {
            ...
            // Quaternion prior every step (soft)
            if (i % 5 === 0) {
        ...
        // With GPS updates at 10 Hz and 2m noise, expect well under 5m error
        expect(posError).toBeLessThan(5.0);

        // Velocity should track well
        const state = eskf.p;
    ...
    it("covariance decreases with more GPS updates", () => {
        ...
        for (let i = 0; i < traj.length; i++) {
            const pose = traj[i];
            const accelBody = [0, 0, 9.80665];  // hover accel
            const gyro = [0, 0, 0];
    ...
    it("rejects measurements outside the chi-square gate", () => {
        ...
        // Let the filter converge for 1 second
        ...
        // Inject a gross outlier (100m off) — should be rejected
