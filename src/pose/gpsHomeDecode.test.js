...
const BFL = path.resolve(__dirname, "../../../planv5/blackbox/acro1/LOG00007.BFL");
const have = () => { try { fs.accessSync(BFL, fs.constants.R_OK); return true; } catch { return false; } };

// Documented ground truth (degrees / metres MSL).
const HOME = { lat: 48.4023468, lon: -71.1696256, alt: 134.2 };
...
describe("GPS home decode (H-frame) — acro1/LOG00007.BFL", () => {
    it("decodes home from the H-frame via getGPSHome() and ingestion", async () => {
        ...
        // (1) The viewer accessor returns the decoded H-frame home (not null).
        const home = fl.getGPSHome();
        ...
        // (2) Ingestion surfaces the same home (the bug it guards: main-frame read → null).
        ...
        // (3) Drone was static at home pre-arm ⇒ home coincides with the first GPS fix.
        // Convert the small lat/lon difference to metres (~111 km/deg lat).
        const g0 = d.gps[0];
        const dN = (g0.lat - home.lat) * 111320;
        const dE = (g0.lon - home.lon) * 111320 * Math.cos(home.lat * Math.PI / 180);
        const sep = Math.hypot(dN, dE);
