...
describe("estimatorLoop — end-to-end forward + RTS", () => {
    it("recovers a straight trajectory from noisy GPS with smoothing", () => {
        const traj = generateStraightTrajectory({
            ...
            speedMs: 10,
            headingDeg: 90,
            freqHz: 200, // higher rate for IMU simulation
        });

        const rng = createRng(5555);

        // Build IMU samples (level drone, heading 90°)
        ...
        // Last pose should be travelling east (~35m east after 5s at 10m/s → 0.0004°)
        const lastLon = poses[poses.length - 1].lon;
    ...
    it("produces decreasing sigmaPos over time as GPS accumulates", () => {
        ...
        // Position sigma should be reasonable (not explosive)
        const endSigma = poses[poses.length - 1].sigmaPos;
        expect(endSigma).toBeGreaterThan(0);
        expect(endSigma).toBeLessThan(10); // should be well under 10m
    });
});
