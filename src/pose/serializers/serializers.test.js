...
import { poseTrackToKml } from "./kmlSerializer.js";
import { poseTrackToJson, poseTrackFromJson } from "./jsonSerializer.js";
import { poseTrackToCsv } from "./csvSerializer.js";
import { poseTrackToGpx } from "./gpxSerializer.js";

function makeSample(tUs, p, v, q, lla) {
    return {
        tUs, p, v, q, lla,
        covPos: [[0.01, 0, 0], [0, 0.01, 0], [0, 0, 0.01]],
        covAtt: [[0.001, 0, 0], [0, 0.001, 0], [0, 0, 0.001]],
    };
}
...
function makeTrack() {
    const samples = [
        ...
        makeSample(500000, [5,0,0], [10,0,0], [0.9659, 0, 0, 0.2588], { lat: 48.408045, lon: -71.16394, alt: 200 }),
    ];
...
describe("Serializers", () => {
    ...
    it("KML with no triads still emits path", () => {
        const track = makeTrack();
        const kml = poseTrackToKml(track, { showTriads: false, showPath: true, showRawGps: false });
    ...
    it("CSV produces valid tabular output", () => {
        ...
        expect(csv).toContain("tMs,lat,lon,altMsl,qw,qx,qy,qz,vn,ve,vd,sigmaPos,sigmaAtt");
        expect(csv).toContain("48.408000"); // first lat
        expect(csv.trim().split("\n")).toHaveLength(3); // header + 2 rows
    });

    it("GPX produces valid XML", () => {
        const track = makeTrack();
